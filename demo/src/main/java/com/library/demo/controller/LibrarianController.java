...
public class LibrarianController {
	...
	public LibrarianController(LoanService loanService, BookService bookService, InventoryService inventoryService,
	...
	public ResponseEntity<Librarian> addLibrarian(@RequestBody Librarian librarianRequest) {
		Librarian librarianResponse = librarianService.addLibrarian(librarianRequest);
		return ResponseEntity.ok().body(librarianService.getLibrarian(librarianResponse.getId()));
	}

	@GetMapping(path = "/{id}", produces = { MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<Librarian> getLibrarian(@PathVariable Integer id) {
		return ResponseEntity.ok().body(librarianService.getLibrarian(id));
	}
}
